...
**CRITICAL - Store PDF data in document store:**
After extractPDF completes successfully, store document metadata for Plan 03-03 to use:
```typescript
...
// Store extracted data (enables TOC navigation in Plan 03-03)
...
// Then continue with word grouping and RSVP initialization
...
Follow existing patterns: client component with "use client", Card layout, Stone theme styling. Maintain vertical spacing and centered layout from current design.
  ...
  <done>User can upload PDF, see extraction progress, RSVP display shows PDF text, and document store contains extracted data</done>
</task>
