...
must_haves:
  truths:
    ...
    - "User's WPM and word grouping settings persist across browser refresh"
  artifacts:
...
Layout: Place after Play/Pause button, before restart button (from 04-01).
  </action>
  <verify>
1. Build succeeds: `npm run build`
...
<task type="auto">
  <name>Task 4: Verify settings persistence (UI-03)</name>
  <files>store/settings-store.ts</files>
  <action>
Verify that settings persistence (implemented in Phase 2 via Zustand persist middleware) works correctly across browser sessions.

NO CODE CHANGES NEEDED - settings-store.ts already has persist middleware wrapping the store.

This task is verification-only to confirm UI-03 requirement fulfillment.
  </action>
  <verify>
1. Test WPM persistence:
   - Open application in browser
   - Set WPM to 450 (non-default value)
   - Refresh browser (F5)
   - Confirm WPM still shows 450 (not default 300)
   - Check localStorage → Application tab → rsvp-settings
   - Should contain {"wpm": 450, "wordsPerGroup": 1}

2. Test word grouping persistence:
   - Set word grouping to 3 words per flash
   - Set WPM to 500
   - Close browser tab completely
   - Reopen application in new tab
   - Confirm both settings retained: WPM = 500, wordsPerGroup = 3
   - Check localStorage → rsvp-settings contains both values

3. Test settings survive browser restart:
   - Set WPM to 600, wordsPerGroup to 2
   - Close ALL browser windows (quit browser)
   - Reopen browser, navigate to application
   - Confirm settings preserved: WPM = 600, wordsPerGroup = 2
  </verify>
  <done>
- WPM setting persists across browser refresh (confirmed)
- Word grouping setting persists across browser refresh (confirmed)
- Both settings persist across browser close/reopen (confirmed)
- localStorage contains rsvp-settings with correct serialized state (confirmed)
- UI-03 requirement verified as fulfilled via Phase 2 implementation
  </done>
</task>
...
4. **Settings persistence works (UI-03):**
- Set WPM to 450, word grouping to 3
- Refresh browser
- Confirm both settings retained
- Check localStorage → rsvp-settings contains correct values
...
- WPM setting persists across browser sessions (UI-03)
- Word grouping setting persists across browser sessions (UI-03)
...
- Settings store persist middleware continues to function (no regression)
...
- UI-03: System persists user settings across sessions ✓ (verified via settings-store from Phase 2)
</success_criteria>
...
- Settings persistence verification results (UI-03)
